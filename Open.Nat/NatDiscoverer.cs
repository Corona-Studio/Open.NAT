...
namespace Open.Nat
{
    ...
    public class NatDiscoverer
    {
        ...
        {
            foreach (var device in Devices.Values) device.ReleaseAll();
        }

        internal static void ReleaseSessionMappings()
        {
            foreach (var device in Devices.Values) device.ReleaseSessionMappings();
        }
        ...
        private static void RenewMappings(object state)
        {
            ...
            {
                foreach (var device in Devices.Values) await device.RenewMappings();
            });
#endif
        }
    }
